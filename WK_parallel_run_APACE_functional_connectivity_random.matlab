...
% Preparation
%
% Code that can be used to compute the largest connected component (based on network-based statistics NBS) (Zalesky et al. 2010) Neuroimage Volume 53, Issue 4, December 2010, Pages 1197-1207
%
...
addpath(genpath('~/APACE-0.1.4_reloaded/'))

base = '~/participants/';
conn_paths = '';
kinship_list = 'zygosity.csv'; % see info further below 
results_dir = '~/results_ACE/permutations/';
mask = '';
...
ACEfit_Par.P_nm      = A; % fullfile(base,conn_paths);
                                          % A list of images or other data
                                          % specification; see
                                          % FileFormats.txt

ACEfit_Par.InfMx     = fullfile(base,kinship_list);
...
ACEfit_Par.ResDir    =  results_dir;      % Set result folder
...
ACEfit_Par.Pmask     = mask; % neurosynth pain mask
                                          % Brain mask image (default:
                                          % whole volume)
...
ACEfit_Par.nPerm     = 0;                % Number of permutations
                                          % (default: 1000)

ACEfit_Par.nBoot     = 0;                % Number of bootstrap replicates
                                          % (default: 1000)

ACEfit_Par.nParallel = 1;                % Number of parallel runs
                                          % (default: 1, without
                                          % parallelization)
